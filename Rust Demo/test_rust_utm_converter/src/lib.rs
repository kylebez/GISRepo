...
mod python_bindings {
    ...
    #[pyfunction]
    fn to_utm(latitude: f64, longitude: f64, zone: u8) -> PyResult<(f64, f64, f64)> {
        Ok(crate::to_utm_wgs84(latitude, longitude, zone))
    }
    ...
    #[pyfunction]
    fn to_lat_lon(easting: f64, northing: f64, zone_num: u8, zone_letter: char) -> PyResult<(f64, f64)> {
        crate::wsg84_utm_to_lat_lon(easting, northing, zone_num, zone_letter).map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))
    }

    #[pyfunction]
    fn lat_to_zone_letter(latitude: f64) -> PyResult<Option<char>> {
        Ok(crate::lat_to_zone_letter(latitude))
    }

    #[pyfunction]
    fn lat_lon_to_zone_number(latitude: f64, longitude: f64) -> PyResult<u8> {
        Ok(crate::lat_lon_to_zone_number(latitude, longitude))
    }

    #[pymodule]
    fn test_rust_utm_converter(m: &Bound<'_, PyModule>) -> PyResult<()> {
        m.add_function(wrap_pyfunction!(to_utm, m)?)?;
        m.add_function(wrap_pyfunction!(to_utm_no_zone, m)?)?;
        m.add_function(wrap_pyfunction!(to_lat_lon, m)?)?;
        m.add_function(wrap_pyfunction!(lat_to_zone_letter, m)?)?;
        m.add_function(wrap_pyfunction!(lat_lon_to_zone_number, m)?)?;
        Ok(())
}
...
impl std::error::Error for WSG84ToLatLonError {
    fn description(&self) -> &str {
        match self {
            ...
            WSG84ToLatLonError::InvalidLatitude => {"Invalid latitude value"
            },
            WSG84ToLatLonError::InvalidLongitude => {"Invalid longitude value"
            },
            WSG84ToLatLonError::InvalidZone => {"Invalid zone value"
            }
        }
    }
}
...
impl std::fmt::Display for WSG84ToLatLonError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            WSG84ToLatLonError::EastingOutOfRange => write!(f, "Easting out of range, must be between 100000 and 999999"),
            WSG84ToLatLonError::NorthingOutOfRange => write!(f, "Northing out of range, must be between 0 and 10000000"),
            WSG84ToLatLonError::ZoneNumOutOfRange => write!(f, "Zone num out of range, must be between 1 and 60"),
            WSG84ToLatLonError::ZoneLetterOutOfRange => write!(f, "Zone letter out of range, must be between C and X"),
            WSG84ToLatLonError::InvalidLatitude => write!(f, "Invalid latitude value"),
            WSG84ToLatLonError::InvalidLongitude => write!(f, "Invalid longitude value"),
            WSG84ToLatLonError::InvalidZone => write!(f, "Invalid zone value")
        }
    }
}
